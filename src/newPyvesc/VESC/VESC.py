from newPyvesc.protocol.interface import encode_request, encode, decode
from newPyvesc.VESC.messages import *
import time
import threading
...
class VESC(object):
    def __init__(self, serial_port, has_sensor=False, start_heartbeat=True, baudrate=115200, timeout=0.05):
        ...
        if version is None:
            print("Version is None type")
        else:
            print("Version is NOT NONE tupe")
            print(type(version))
        if version != "None" or version != 'None':
             if int(version.split('.')[0]) < 3 or version != "None" or version != 'None':
