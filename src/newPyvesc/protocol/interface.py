import newPyvesc.protocol.base
import newPyvesc.protocol.packet.codec
...
def decode(buffer):
    ...
    """
    msg_payload, consumed = newPyvesc.protocol.packet.codec.unframe(buffer)
    if msg_payload:
        return newPyvesc.protocol.base.VESCMessage.unpack(msg_payload), consumed
    else:
...
def encode(msg):
    ...
    :rtype: bytes
    """
    msg_payload = newPyvesc.protocol.base.VESCMessage.pack(msg)
    packet = newPyvesc.protocol.packet.codec.frame(msg_payload)
    return packet
...
def encode_request(msg_cls):
    ...
    :rtype: bytes
    """
    msg_payload = newPyvesc.protocol.base.VESCMessage.pack(msg_cls, header_only=True)
    packet = newPyvesc.protocol.packet.codec.frame(msg_payload)
    return packet
